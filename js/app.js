...
const state={
  projects:{},employees:{},assignments:{},customCountries:{},
...
function seedLocal(){state.customCountries=state.customCountries||{};if(!Object.keys(state.employees).length)state.employees=Object.fromEntries(SEED_EMPLOYEES.map(e=>[e.id,{...e,country:normCountry(e.country)}]));if(!Object.keys(state.projects).length)state.projects=structuredClone(defaultProjects);if(!state.country||!countries().includes(state.country))state.country=countries()[0]||"Malaysia"}
function serialise(){return JSON.stringify({projects:state.projects,employees:state.employees,assignments:state.assignments,customCountries:state.customCountries})}
...
async function saveState(){const payload={version:2,projects:state.projects,employees:state.employees,assignments:state.assignments,customCountries:state.customCountries,updatedAt:firebase.firestore.FieldValue.serverTimestamp()};await stateRef.set(payload,{merge:false});lastSaved=serialise();setSync("Saved online","ok")}
...
async function initialiseFirestore(){
  ...
  try{
    ...
    if(snap.exists){
      ...
      state.projects=d.projects||state.projects;state.employees=d.employees||state.employees;state.assignments=d.assignments||state.assignments;state.customCountries=d.customCountries||state.customCountries;
    ...
    stateRef.onSnapshot(s=>{if(!s.exists)return;const d=s.data(),incoming=JSON.stringify({projects:d.projects||{},employees:d.employees||{},assignments:d.assignments||{},customCountries:d.customCountries||{}});if(incoming===lastSaved)return;applyingRemote=true;state.projects=d.projects||{};state.employees=d.employees||{};state.assignments=d.assignments||{};state.customCountries=d.customCountries||{};lastSaved=serialise();applyingRemote=false;seedLocal();renderAll();setSync("Updated online","ok")},e=>setSync(e.code||"Sync error","error"));
...
function updateHeader(){
  ...
  ["calendar","manpower","projects"].forEach(v=>$(v+"View").hidden=state.view!==v);
}
function renderAll(){seedLocal();renderCountryTree();updateHeader();renderCalendar();renderManpower();renderProjects();populateAssignmentSelects()}
...
function formatAssignmentPeriod(projectId,employeeIds,targetDate){
  const selected=new Set(employeeIds),all=filteredAssignments().filter(a=>a.projectId===projectId&&selected.has(a.employeeId)).map(a=>a.date).sort();
  if(!all.length)return targetDate;
  const unique=[...new Set(all)],targetIndex=unique.indexOf(targetDate);if(targetIndex<0)return targetDate;
  let start=targetIndex,end=targetIndex;
  const nextDay=d=>{const x=new Date(d+"T00:00");x.setDate(x.getDate()+1);return dateKey(x)};
  while(start>0&&nextDay(unique[start-1])===unique[start])start--;
  while(end<unique.length-1&&nextDay(unique[end])===unique[end+1])end++;
...
function openEmployee(id=null){const e=id?state.employees[id]:null;$("employeeDialogTitle").textContent=e?"Edit Manpower":"Add Manpower";$("employeeId").value=e?.id||"";$("employeeName").value=e?.name||"";$("employeePosition").value=e?.position||"";const dept=e?.department||((e?.team||"project")==="service"?"Service":"Project");$("employeeDepartment").value=[...$("employeeDepartment").options].some(o=>o.value===dept)?dept:"Other";$("employeeStatus").value=e?.status||"Available";$("employeeCountry").value=state.country;$("employeeCompany").value=e?.company||"";$("employeeDialog").showModal()}
...
$("employeeForm").onsubmit=e=>{e.preventDefault();const id=$("employeeId").value||uid("emp"),name=$("employeeName").value.trim(),initials=name.split(/\s+/).map(x=>x[0]).join("").slice(0,2).toUpperCase(),department=$("employeeDepartment").value,team=department==="Service"?"service":"project";state.employees[id]={id,name,initials,position:$("employeePosition").value.trim(),team,department,status:$("employeeStatus").value,country:state.country,company:$("employeeCompany").value.trim(),active:$("employeeStatus").value!=="Inactive"};$("employeeDialog").close();scheduleSave();renderAll()}
...
$("primaryActionBtn").onclick=()=>openAssignment();$("quickAddBtn").onclick=()=>state.view==="calendar"?openAssignment():state.view==="manpower"?openEmployee():openProject();$("addEmployeeBtn").onclick=()=>openEmployee();$("addProjectBtn").onclick=()=>openProject();$("employeeSearch").oninput=renderManpower;
